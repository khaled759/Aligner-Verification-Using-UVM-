`ifndef CFS_APB_IF_SV
  ...
  interface cfs_apb_if(input pclk);
    ...
    // rule one: penable must be asserted to 1 after 1 cycle of entring setup phase
    ...
    property penable_entring_access_phase_p;
      @(posedge pclk) disable iff(!preset_n || !has_checks)
      setup_phase_s |=> penable == 1;
    endproperty

    PENABLE_ENTRING_ACCESS_PHASE_A: assert property(penable_entring_access_phase_p) else 
      $error("PENABLE when entring \"Access Phase\" is not equal to 1");

    // rule two: penable must be deasserted 
    property penable_exiting_access_phase_p;
      @(posedge pclk) disable iff(!preset_n || !has_checks)
      access_phase_s and (pready) |=> penable == 0;
    endproperty
    ...
    // rule three: all input signals must be stable within access phase
    property penable_stable_at_access_phase_p;
      @(posedge pclk) disable iff(!preset_n || !has_checks)
      access_phase_s |-> penable == 1;
    endproperty

    PENABLE_STABLE_AT_ACCESS_PHASE_P: assert property(penable_stable_at_access_phase_p) else
      $error("PENABLE at \"Access Phase\" is not stable");
    ...
    // rule four: APB signals can not have unknown values (x, z)
